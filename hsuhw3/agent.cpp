...
int main(int argc, char* argv[]){
    ...
    if(argc != 7){
        fprintf(stderr,"foramt: %s <sender IP> <recv IP> <sender port> <agent port> <recv port> <loss_rate>\n", argv[0]);
        fprintf(stderr, "example: ./agent 127.0.0.1 127.0.0.1 3333 3334 3335 0.03\n");
        exit(1);
    } 
    else{
        setIP(ip[0], argv[1]);
